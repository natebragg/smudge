...
instance (Graph gr) => Passable (NoTransientAnyState gr) where
    ...
    accumulate (_, n , EnterExitState {st = StateAny _}, o) a = mappend (NoTransientAnyState [n] [(n, n') | (Happening (EventEnter _) _ _, n') <- o]) a
    accumulate (i, n',                                _, _) a = mappend (NoTransientAnyState []  [(n, n') | (Happening (EventEnter _) _ _, n ) <- i]) a
