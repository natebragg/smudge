...
data Include = Sys FilePath | Rel FilePath
...
instance Backend CStaticOption where
    ...
    generate os cfg gswust outputTarget = do
        ...
        where (gs, aliases, syms_0) = gswust
              ...
              genIncludes :: [Include] -> FilePath -> IO [Char]
              genIncludes includes includer = liftA concat $ sequence $ map (mkInclude includer) includes

              smearify :: SmudgeIR QualifiedName -> SmudgeIR QualifiedName
              smearify ir = if smear then filter (not . smearish) ir ++ [main] else ir
                where smearish :: Def QualifiedName -> Bool
                      smearish (DataDef _) = False
                      ...
                      main = FunDef (qualify "main") [] (External, Void :-> int) [] [
                            ...
                            ExprS $ FunCall (qualify "SRT_run") [],
                            ExprS $ FunCall (qualify "SRT_stop") [],
                            Return Null]

              mkInclude :: FilePath -> Include -> IO [Char]
              mkInclude        _ (Sys include) =
                  return $ concat ["#include <", include, ">\n"]
              mkInclude includer (Rel include) =
                do
                  relativeInclude <- relPath (takeDirectory includer) include
                  return $ concat ["#include \"", relativeInclude, "\"\n"]

              smearHandlers = ["SRT_HANDLERS(" ++ (extractWith seq (mangle mangleIdentifier) $ rename aliases $ qualify smName) ++ ")\n" | (StateMachine smName, _) <- gs]

              srcLeader includes fp =
                do
                  gennedIncludes <- genIncludes includes fp
                  return $ concat $ [ward, gennedIncludes, "\n"] ++ if smear then smearHandlers ++ ["\n"] else []
